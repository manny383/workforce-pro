...
import { AlertCircle, BadgeCheck, CalendarCheck, Clock, History as HistoryIcon, LoaderCircle, MapPin, Timer, User } from 'lucide-react';
...
export const ManagerDashboardView = ({ session, onAction }: { session: Session; onAction: (v: View) => void }) => {
  ...
  return <div className="space-y-10 pb-32">
    ...
    <div className="grid gap-6 md:grid-cols-4">
      <button onClick={() => onAction('today-active')} className="relative overflow-hidden rounded-2xl bg-surface-container-low p-8 text-left transition hover:ring-2 hover:ring-primary/30 md:col-span-2"><span className="inline-flex items-center gap-2 rounded-full bg-tertiary/10 px-3 py-1 text-[10px] font-bold uppercase text-tertiary"><span className="h-2 w-2 animate-pulse rounded-full bg-tertiary" /> En vivo</span><h3 className="mt-4 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Trabajadores activos</h3><div className="flex items-baseline gap-3"><span className="font-headline text-6xl font-extrabold text-primary">{data.summary.activos}</span><span className="text-lg text-on-surface-variant">/ {data.summary.empleados}</span></div><User className="absolute -bottom-5 -right-5 opacity-10" size={150} /></button>
      <button onClick={() => onAction('today-late')} className="rounded-2xl bg-surface-container-lowest p-6 text-left shadow-sm transition hover:ring-2 hover:ring-primary/30"><AlertCircle className="mb-4 text-error" /><h3 className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Retardos hoy</h3><p className="font-headline text-4xl font-bold">{data.summary.retardos}</p></button>
      <button onClick={() => onAction('today-absent')} className="rounded-2xl bg-surface-container-lowest p-6 text-left shadow-sm transition hover:ring-2 hover:ring-primary/30"><User className="mb-4 text-outline" /><h3 className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Ausentes hoy</h3><p className="font-headline text-4xl font-bold">{data.summary.ausentes}</p><p className="mt-4 text-xs text-on-surface-variant">{data.summary.presentes} de {data.summary.programados} programados</p></button>
    </div>
    <div className="grid gap-8 lg:grid-cols-12">
      <div className="space-y-8 lg:col-span-8">
        ...
        <section><h3 className="mb-6 font-headline text-xl font-bold text-primary">Control administrativo</h3><div className="grid gap-4 md:grid-cols-5">{[{ label: 'Hoy', sub: `${data.summary.programados} programados`, icon: CalendarCheck, view: 'today' }, { label: 'Ubicaciones', sub: `${data.summary.locaciones} activas`, icon: MapPin, view: 'locations' }, { label: 'Empleados', sub: `${data.summary.empleados} activos`, icon: BadgeCheck, view: 'team' }, { label: 'Turnos', sub: 'Horarios base', icon: Timer, view: 'shifts' }, { label: 'Reportes', sub: 'Historial de asistencia', icon: HistoryIcon, view: 'history' }].map((item) => <button key={item.label} onClick={() => onAction(item.view as View)} className="flex flex-col items-start rounded-2xl border border-outline-variant/20 bg-surface-container-lowest p-6 hover:border-primary"><item.icon className="mb-4 text-primary" /><span className="font-bold">{item.label}</span><span className="mt-1 text-xs text-on-surface-variant">{item.sub}</span></button>)}</div></section>
      </div>
